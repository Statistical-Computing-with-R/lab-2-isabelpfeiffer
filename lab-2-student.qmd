...
class(surveys)
mode(surveys)
# https://libraryguides.mcgill.ca/c.php?g=699776&p=4968544#:~:text=Data%20Types%20in%20R,-There%20are%20different&text=These%20data%20types%20can%20be,()%20or%20typeof()%20functions.
```
<!-- You can either write code here or in the code chunk for #1, but I **must** see the code you used to address this question!  -->
...
ggplot( data = surveys, 
        ...
        geom_jitter(width = .1, alpha = 0.1, color = "steelblue")
        labs(title = "Species vs. Weight",
        subtitle = " weight")
        theme(axis.text.x = "species")
...
We reject the null hypothesis. Based off of the small p value of 2e-16, we have evidence to suggest that at least one rodent species has a different population mean weight. 
